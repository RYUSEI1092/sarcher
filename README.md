...
## 開発のフロー
開発する際のフローを記載します。    
開発には`git-flow`という手法を使いますが、難しく考える必要はなく、  
1. developからfeatureブランチを作成して開発
2. 開発が終わればfeatureブランチをdevelopにマージする

という手順になります。  

基本的には下記のコマンドで上記を運用していくことになります。    
※Issue番号が999の場合を例として記載します。  
#999となっている箇所は実際のIssue番号と置き換えて実行してください  
1. git stash
2. git checkout develop
3. git pull origin develop
4. git checkout -b feature/999
5. 〜開発〜
6. git add .
7. git commit -m "feat: 変更した内容を記載する"
8. git push origin feature/999
9. 1に戻る

## 必須コマンド
インターン研修で最低限必要なコマンドとコマンドの説明を記述します。

### git checkout <ブランチ名>
現在作業しているブランチを切り替えます

### git checkout -b <ブランチ名>
現在作業しているブランチをベースに新しいブランチを作成します

### git add .
変更したファイルをindex(仮登録)状態にします。  
この状態にすることで、次のcommitで登録状態にすることができます。  
`git add .` の`.`は現在のファイルより下位のファイルを全て登録するという意味なので、  
必ずcloneしたフォルダの一番上位のフォルダで実行してください。  
また、`git add ./aaa/xxx/yyy.vue` というように1つのファイルをindex状態にすることもできます。  

indexについての説明  
https://backlog.com/ja/git-tutorial/intro/04/

### git commit -m "コメント"
`git add .` でindex(仮登録)状態にしたファイルをcommit(登録確定)に変更します。  
この状態になった場合、元に戻すことはできないと考えたほうが良いです。  
`-m "コメント"` のオプションをつけることでコミットにメッセージをつけることができます。  
コメントは必須でつけてください。  

### git push origin <ブランチ名>
ローカルリポジトリで編集してcommitしたファイルをリモートリポジトリにアップロードします。  
このコマンドを実行することで、Githubにアップロードされ他の人からも見えるようになります。  

### git pull origin <ブランチ名>
リモートリポジトリに上がっているブランチをローカルリポジトリにダウンロードします。  
主にリモートリポジトリのdevelopが更新された時に、ローカルリポジトリのdevelopを最新化するために使用します。  

## 任意コマンド
必須ではないですが、困った時に使用することがあるコマンドです。

### git branch -a
ブランチの一覧を表示します。  
リモートリポジトリは `remotes/origin/` から始まります。  
ローカルブランチは上記以外から始まるブランチです。  

### git stash
index状態やcommit済みでない変更点を退避します。  
作業中に別のブランチで作業したくなった時に使用したりします。  
復元は番号で行います。  

1. `git stash list` でstashの一覧を確認
```
# git stash list
---
stash@{0}: WIP on aaaaaa
stash@{1}: WIP on xxxxxx
stash@{2}: WIP on yyyyyy
---
```
2. 復元したいstashの `stash@{x}` の数字を引数に `git stash apply` を実行する
```
# git stash apply 1
---
stash@{1}: WIP on xxxxxx が復元される
---
```
